...
def check_constraint_5(_ds: np.ndarray, _supply_charges: dict[int, tuple[int, int]]) -> None:
  sum_of_cols = _ds.sum(axis=0)

  for i in range(100):
    a = sum_of_cols[i]
    b = get_max_supply(_supply_charges[i])
    assert round(a,3) <= b, f'Constraint 5 failed for col {i}. Sum={a} - Smax={b}'
...
def check_constraint_6(_ds: np.ndarray, demand_values: list[float]) -> None:
  ...
  for i in range(4096):
    a = sum_of_rows[i]
    b = demand_values[i]
    assert abs(a-b) < 10**-2, f'Constraint 6 failed for row {i}. Sum={a} - Demand={b}'
...
def get_overall_cost(_ds: np.ndarray,
                     _supply_charges: dict[int, tuple[int, int]],
