...
sorted_demand_points = [
    (int(dp.demand_point_index), dp.value) for _, dp in demand_points.sort_values('value', ascending=False).iterrows()
]
...
def offspring_generator(prob: float) -> dict[int, tuple[int, int]]:
    ...
    for i in range(100):
        ...
        temp_dict[i] = (previous_charges[i][0] + random_scs, previous_charges[i][1] + random_fcs)
    return temp_dict


def crossover(parent1: dict, parent2: dict, start: int, end:int) -> tuple[dict, dict]:
    offspring1 = {}
    offspring2 = {}
...
def crossover_twopoints(parent1: dict, parent2: dict, start: int, end:int) -> tuple[dict, dict]:
  offspring1 = {}
  offspring2 = {}
...
def mutate(offspring: dict, num_of_charges: int) -> dict:
    for _ in range(num_of_charges):
        ...
        random_scs = random.randint(0, available_parking_slots)
        random_fcs = random.randint(0, available_parking_slots - random_scs)
        offspring[idx] = (previous_charges[idx][0] + random_scs, previous_charges[idx][1] + random_fcs)
    return offspring
...
while generation <= GENERATIONS:
	...
	for _ in range(s):
		...
		# Fitness calculation
		new_generation.append((child1, fitness_function(
            child1,
            sorted_demand_points,
            reverse_proximity,
            parking_slots,
		...
		new_generation.append((child2, fitness_function(
            child2,
            sorted_demand_points,
            reverse_proximity,
            parking_slots,
...
best_ds = distribute_supply(best_population, sorted_demand_points, reverse_proximity)
