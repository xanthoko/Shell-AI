...
def create_submission_data() -> pd.DataFrame:
    ...
    half_19_chargers.value = pd.concat([chargers_19.scs, chargers_19.fcs
                                        ]).astype("float64").tolist()
    # Overwrite with the calculated 2019 DS values
    half_19_demand_points.value = ds_19.reshape(ds_19.shape[0] * ds_19.shape[1],
                                                -1).squeeze()

    # Overwrite with the calculated 2020 supply points
    half_20_chargers.value = pd.concat([chargers_20.scs, chargers_20.fcs
                                        ]).astype("float64").tolist()
    # Overwrite with the calculated 2020 DS values
    half_20_demand_points.value = ds_20.reshape(ds_20.shape[0] * ds_20.shape[1],
                                                -1).squeeze()

    result = pd.concat([
        half_19_chargers, half_19_demand_points, half_20_chargers,
        half_20_demand_points
    ])
